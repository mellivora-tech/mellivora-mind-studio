...
interface MarketCardProps {
  ...
  name: string;
  price: string;
  change: string; // e.g. "+1.24%" or "-0.50%"
  type?: 'index' | 'stock' | 'crypto';
}

const MarketCard: Component<MarketCardProps> = (props) => {
  const isUp = () => props.change.startsWith('+');

  return (
    <div class="bg-surface rounded-lg p-4 border border-base hover:border-brand/50 transition-colors cursor-pointer group relative overflow-hidden">
      {/* Top Row: Icon & Info */}
      ...
      <div class="absolute bottom-0 left-0 right-0 h-16 opacity-20 pointer-events-none">
        <Show 
            when={isUp()} 
            fallback={
                <svg class="w-full h-full text-down" preserveAspectRatio="none" viewBox="0 0 100 40">
