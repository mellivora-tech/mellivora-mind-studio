...
const Navbar: Component = () => {
  ...
  return (
    <nav class="sticky top-0 z-50 w-full border-b border-base bg-canvas/95 backdrop-blur supports-[backdrop-filter]:bg-canvas/80">
      <div class="mx-auto flex h-16 max-w-[1440px] items-center justify-between px-4 sm:px-6 lg:px-8">
            ...
            {navLinks.map((link) => (
              <a
                ...
                class={`text-sm font-medium transition-colors hover:text-white ${
                  link.active ? 'text-white' : 'text-secondary'
                }`}
              >
                {link.name}
              </a>
        ...
        <div class="hidden flex-1 items-center justify-center px-8 lg:flex">
          <div class="relative w-full max-w-sm">
            ...
            <input
              ...
              class="block w-full rounded-full border border-base bg-surface py-1.5 pl-10 pr-3 text-sm text-primary placeholder-secondary focus:border-brand focus:outline-none focus:ring-1 focus:ring-brand"
            />
          </div>
        </div>
        ...
        <div class="flex items-center gap-4">
          ...
          {/* Mobile Menu Toggle */}
          <button
            class="md:hidden rounded-full p-2 text-primary hover:bg-surface"
        ...
        <div class="md:hidden border-t border-base bg-canvas p-4">
          <div class="flex flex-col gap-4">
            {navLinks.map((link) => (
              <a
                href={link.href}
                class={`text-base font-medium px-2 py-1 ${
                  link.active ? 'text-white bg-surface rounded' : 'text-secondary'
                }`}
              >
                {link.name}
              </a>
